<template>
      ...
      <div class="row pb-2 pb-sm-4">
        ...
        <aside class="col-lg-3">
          <div class="offcanvas-lg offcanvas-start" id="shopSidebar">
            ...
            <div class="offcanvas-body pt-2 pt-lg-0 pe-lg-4">
              ...
              <!-- Category (checkboxes) -->
              <div class="pb-2">
                <div class="form-check" v-for="(count, category) in categoryFilters" :key="category">
                  ...
                  <label class="form-check-label d-flex align-items-center" :for="'category-'+category">
                    <span class="text-nav fw-medium">{{ category }}</span>
              ...
              <div class="pb-2">
                <div class="form-check" v-for="(count, market) in marketFilters" :key="market">
                  <input class="form-check-input" type="checkbox" :value="market" :id="'market-'+market" 
                         v-model="selectedMarketFilters" @change="applyFilters">
                  <label class="form-check-label d-flex align-items-center" :for="'market-'+market">
                    <span class="text-nav fw-medium">{{ market }}</span>
                    <span class="fs-xs text-body-secondary ms-auto">{{ count }}</span>
                  </label>
                </div>
              </div>
        ...
        <div class="col-lg-9">
          ...
          <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-4">
            <!-- Item -->
            <div class="col pb-2 pb-sm-3" v-for="product in filteredProducts" :key="product.id">
  ...
  export default {
    ...
    data() {
      return {
        // All products data (this would come from an API in a real app)
        allProducts: [
          {
            id: 1,
          ...
          },
          {
            ...
            price: 29.99,
            image: "@/assets/img/shop/products/01.png", // Reusing image for example
            category: "Home",
            market: "samsung"
          },
          {
            ...
            name: "Casual T-shirt",
            price: 24.50,
            image: "@/assets/img/shop/products/02.png", // Reusing image for example
          ...
          },
          {
            ...
            name: "Digital watch",
            price: 68.00,
            image: "@/assets/img/shop/products/03.png", // Reusing image for example
            category: "electonic",
            market: "samsung"
          },
          {
    ...
    computed: {
      ...
      // Dynamically calculate market filter counts
      marketFilters() {
        const counts = {};
        this.allProducts.forEach(product => {
          counts[product.market] = (counts[product.market] || 0) + 1;
        });
        return counts;
      },
    ...
    methods: {
      ...
      removeFilter(filter) {
        if (filter.type === 'category') {
          this.selectedCategoryFilters = this.selectedCategoryFilters.filter(cat => cat !== filter.value);
        } else if (filter.type === 'market') {
          this.selectedMarketFilters = this.selectedMarketFilters.filter(market => market !== filter.value);
